...
namespace DB_Schema_Export_Tool
{
    ...
    /// </remarks>
    internal class DeleteExtraDataRowsScripterGenericT : EventNotifier
    {
        ...
        public DeleteExtraDataRowsScripterGenericT(DBSchemaExporterSQLServer dbSchemaExporter, SchemaExportOptions options)
        {
            ...
            FileSystemInfo deleteExtrasFile)
        {
            ...
            for (var i = 0; i < primaryKeyColumnsInSource.Count; i++)
            {
                ...
                switch (primaryKeyColumnTypes[i])
                {
                    ...
                    case DBSchemaExporterBase.DataColumnTypeConstants.Numeric:
                        var validType = IsDotNetCompatibleDataType(dataExportParams, primaryKeyColumnIndex, out _);
            ...
            IsDotNetCompatibleDataType(dataExportParams, primaryKeyColumnIndices[0], out var systemType);
            ...
            if (textDataType)
            {
                return DeleteExtraRowsUsingPrimaryKey(
                    dataExportParams, queryResults, deleteExtrasFile,
                    primaryKeyColumnsInSource, primaryKeyColumnsInTarget,
                    primaryKeyColumnIndices, primaryKeyColumnTypes, default(string), true);
            }
            ...
            if (systemType == Type.GetType("System.Byte"))
            {
                return DeleteExtraRowsUsingPrimaryKey(
                    dataExportParams, queryResults, deleteExtrasFile,
                    primaryKeyColumnsInSource, primaryKeyColumnsInTarget,
                    primaryKeyColumnIndices, primaryKeyColumnTypes, default(byte));
            }

            if (systemType == Type.GetType("System.Int16"))
            {
                return DeleteExtraRowsUsingPrimaryKey(
                    dataExportParams, queryResults, deleteExtrasFile,
                    primaryKeyColumnsInSource, primaryKeyColumnsInTarget,
                    primaryKeyColumnIndices, primaryKeyColumnTypes, default(short));
            }

            if (systemType == Type.GetType("System.Int32"))
            {
                return DeleteExtraRowsUsingPrimaryKey(
                    dataExportParams, queryResults, deleteExtrasFile,
                    primaryKeyColumnsInSource, primaryKeyColumnsInTarget,
                    primaryKeyColumnIndices, primaryKeyColumnTypes, default(int));
            }

            if (systemType == Type.GetType("System.Int64"))
            {
                return DeleteExtraRowsUsingPrimaryKey(
                    dataExportParams, queryResults, deleteExtrasFile,
                    primaryKeyColumnsInSource, primaryKeyColumnsInTarget,
                    primaryKeyColumnIndices, primaryKeyColumnTypes, default(long));
            }

            if (systemType == Type.GetType("System.Char"))
            {
                return DeleteExtraRowsUsingPrimaryKey(
                    dataExportParams, queryResults, deleteExtrasFile,
                    primaryKeyColumnsInSource, primaryKeyColumnsInTarget,
                    primaryKeyColumnIndices, primaryKeyColumnTypes, default(char));
            }

            if (systemType == Type.GetType("System.Decimal"))
            {
                return DeleteExtraRowsUsingPrimaryKey(
                    dataExportParams, queryResults, deleteExtrasFile,
                    primaryKeyColumnsInSource, primaryKeyColumnsInTarget,
                    primaryKeyColumnIndices, primaryKeyColumnTypes, default(decimal));
            }

            if (systemType == Type.GetType("System.Single"))
            {
                return DeleteExtraRowsUsingPrimaryKey(
                    dataExportParams, queryResults, deleteExtrasFile,
                    primaryKeyColumnsInSource, primaryKeyColumnsInTarget,
                    primaryKeyColumnIndices, primaryKeyColumnTypes, default(float));
            }

            if (systemType == Type.GetType("System.Double"))
            {
                return DeleteExtraRowsUsingPrimaryKey(
                    dataExportParams, queryResults, deleteExtrasFile,
                    primaryKeyColumnsInSource, primaryKeyColumnsInTarget,
                    primaryKeyColumnIndices, primaryKeyColumnTypes, default(double));
            }

            return false;
        }

        private bool DeleteExtraRowsUsingPrimaryKey<T>(
            DataExportWorkingParams dataExportParams,
            DataSet queryResults,
            FileSystemInfo deleteExtrasFile,
            IReadOnlyCollection<string> primaryKeyColumnsInSource,
            ...
            T defaultValueForType,
            bool textDataType = false)
        {
            if (primaryKeyColumnsInSource.Count > 1)
            {
                return DeleteUsingMultiColumnPrimaryKey(
                    defaultValueForType, dataExportParams, queryResults, deleteExtrasFile,
                    primaryKeyColumnIndices, primaryKeyColumnsInTarget, primaryKeyColumnTypes);
            }

            // Single column primary key
            return DeleteUsingSingleColumnKey(
                defaultValueForType, dataExportParams, queryResults, deleteExtrasFile,
                primaryKeyColumnIndices[0], primaryKeyColumnsInTarget[0], textDataType);
        }

        /// <summary>
        /// Create a SQL script to delete extra rows in a table based on data in each of the table's primary key columns
        /// </summary>
        /// <param name="defaultValueForType">This argument allows .NET to determine type T at runtime</param>
        /// <param name="dataExportParams"></param>
        /// <param name="queryResults"></param>
        /// <param name="deleteExtrasFile"></param>
        ...
        /// <returns>True if successful, false if an error</returns>
        private bool DeleteUsingMultiColumnPrimaryKey<T>(
            ...
            IReadOnlyList<DBSchemaExporterBase.DataColumnTypeConstants> primaryKeyColumnTypes)
        {
            ...
            try
            {
                ...
                var primaryKeyComparisonCriteria = new SortedDictionary<T, List<string>>();
                ...
                foreach (DataRow currentRow in queryResults.Tables[0].Rows)
                {
                    ...
                    var firstPrimaryKeyValue = (T)currentRow[primaryKeyColumnIndices[0]];
                ...
                var currentGroupComparisonCriteria = new Dictionary<T, List<string>>();

                var currentGroupValueCount = 0;
                var startKey = default(T);
                var previousKey = default(T);
        ...
        /// <returns>True if successful, false if an error</returns>
        private bool DeleteUsingSingleColumnKey<T>(
            // ReSharper disable once UnusedParameter.Local
            T defaultValueForType,
            DataExportWorkingParams dataExportParams,
            DataSet queryResults,
            ...
            bool textDataType)
        {
            ...
            try
            {
                var primaryKeyValues = new SortedSet<T>();
                ...
                foreach (DataRow currentRow in queryResults.Tables[0].Rows)
                {
                    ...
                    var columnValue = (T)currentRow[primaryKeyColumnIndex];
...
#pragma warning restore CS0162
        ...
        private string GetCommaSeparatedList<T>(IEnumerable<T> items, bool textDataType, bool pgInsertEnabled)
        {
            if (!textDataType)
        ...
        private string GetFormattedValue<T>(T columnValue, bool textDataType, bool pgInsertEnabled)
        {
            if (textDataType)
            {
        ...
        private string GetFormattedValue<T>(T columnValue, DBSchemaExporterBase.DataColumnTypeConstants dataColumnTypeConstants, bool pgInsertEnabled)
        {
        ...
        private bool IsDotNetCompatibleDataType(DataExportWorkingParams dataExportParams, int columnIndex, out Type systemType)
        {
            var columnInfo = dataExportParams.ColumnInfoByType[columnIndex];

            var columnName = columnInfo.Key;
            systemType = columnInfo.Value;
        ...
        private void WriteMultiColumnPrimaryKeyDeleteQuery<T>(
            StreamWriter writer,
            ...
            T startKey,
            T endKey,
            Dictionary<T, List<string>> currentGroupComparisonCriteria,
